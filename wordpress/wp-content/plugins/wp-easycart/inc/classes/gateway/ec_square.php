...
class ec_square extends ec_gateway{
	...
	*****************************************/
	...
	function get_gateway_response( $gateway_url, $gateway_data, $gateway_headers ){
		...
		$headr[] = 'Square-Version: 2019-06-12';
	...
	function renew_token( ){
		$access_token = get_option( 'ec_option_square_access_token' );
		$refresh_token = get_option( 'ec_option_square_refresh_token' );
		$ch = curl_init( );
		$url = "https://connect.wpeasycart.com/square-v2/refresh.php?access_token=" . $access_token . "&refreshv2=true";
		if( $refresh_token && $refresh_token != '' ){
			$url = "https://connect.wpeasycart.com/square-v2/refresh.php?refresh_token=" . $refresh_token;
		}
		...
		if( $response === false )
			$this->mysqli->insert_response( 0, 1, "SQUARE RENEW TOKEN ERROR", curl_error( $ch ) );
		else
