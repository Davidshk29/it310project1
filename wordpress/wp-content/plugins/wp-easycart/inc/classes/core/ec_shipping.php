...
class ec_shipping{
	...
	public function get_shipping_rate_data( $standard_text, $express_text ){
		...
		if( $this->shipping_method == "price" || $this->shipping_method == "weight" || $this->shipping_method == "quantity" || $this->shipping_method == "percentage" ){
			...
			if( $GLOBALS['ec_cart_data']->cart_data->expedited_shipping == 'shipexpress' ){
				if( $this->express_price > 0 ){
					$rates[] = (object) array(
						...
						'amount'	=> (int) number_format( ( $standard_price + $this->express_price ) * 100, 0, '', '' ),
						'id' 		=> 'shipexpress'
					);
				}
			}
			...
			$rates[] = (object) array(
				'label'		=> $standard_text,
				'amount'	=> (int) number_format( $standard_price * 100, 0, '', '' ),
				'id' 		=> 'standard'
			);
			...
			for( $i=0; $i<count( $this->method_based ); $i++ ){
				if( $GLOBALS['ec_cart_data']->cart_data->shipping_method == $this->method_based[$i][2] ){
					...
					$rates[] = (object) array(
						...
						'amount'	=> (int) number_format( $rate * 100, 0, '', '' ),
